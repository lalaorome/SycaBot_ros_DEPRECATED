...
def main(args=None):
    ...
    # Initialisation : Wait for pose and ask for its initial task
    while jb_client.state[0] == 999. :
            time.sleep(0.1)
            jb_client.get_logger().info('No pose yet, waiting again...\n')
    ...
    if plot :
        ...
        ax[3].legend()
        fig.suptitle('Sycabot_W'+str(jb_client.id))
        plt.legend()
        plt.show()
