...
def main(args=None):
    ...
    for i in range(n_inputs):
        ...
        rclpy.spin_once(idfier)
        states.append(idfier.rob_state)
        ...
        wheels_vel_cmd.append([cmd_vel.right, cmd_vel.left])
    ...
    wheels_vel_cmd = np.array(wheels_vel_cmd)
    
    # Get velocity from measured inputs
    Ts_arr = [time_stamps[i+1]-time_stamps[i] for i in range(len(time_stamps)-1)]
    vel_m = p2vel(states,Ts_arr,n_inputs, wheels_vel_cmd)

    if interpolate :
        f_linear = linear_model.LinearRegression(fit_intercept=False)
        f_angular = linear_model.LinearRegression(fit_intercept=False)
        # f_linear = linear_model.Lasso(alpha=0.01, fit_intercept=False)
        # f_angular = linear_model.Lasso(alpha=0.01, fit_intercept=False)
        # f_linear = linear_model.Ridge(alpha=0.5)
    ...
    if plot :
        ...
        if interpolate :
            ...
            ax[0].scatter(states[:,0],states[:,1], label="position measured")
            ...
            ax[1].plot(times, states[:,2], '-', times, theta[1:], '--')
            ax[1].plot(times, [m.pi for i in range(len(times))], label='pi')
            ax[1].plot(times, [-m.pi for i in range(len(times))], label='-pi')
